...
## Project
Cramele Cotnari scraper for peviitor.ro (Node.js, ESM, Jest)

## 🌱 This Repo Is a Derived Scraper
This repo is a scraper **derived** from the [EPAM template](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper) for **CRAMELE COTNARI S.A.** (CIF: 40321026).
...
- **Only the scraping logic in `scraper/index.js`** (`scrapeBestJobs`, `scrapeEJobs`, `searchANOFM`) is source-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across peviitor.ro scrapers.
...
```bash
gh run view <RUN_ID> --repo TheTatu13/cramele-cotnari-sa-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo TheTatu13/cramele-cotnari-sa-nodejs-scraper --state open`
