...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-cramele-cotnari-jobs.js`, and the deep-validate workflow. |
...
| `tests/validate-cramele-cotnari-jobs.js` | **Company-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
| `tests/unit/index.test.js` | Unit tests for index.js - scrapeBestJobs, scrapeEJobs, searchANOFM, mapToJobModel, transformJobsForSOLR |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real BestJobs/eJobs, ANAF, and Peviitor API |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for BestJobs/eJobs |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape BestJobs/eJobs/ANOFM → transform → upsert → generate docs/jobs.md
